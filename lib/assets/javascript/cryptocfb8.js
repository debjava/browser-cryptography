...
exports.encryptTextWithKey = void 0;
 ...
 * are supported.
 * Code modified by Debadatta Mishra(DD, EMC2)
 */
CryptoJS.mode.CFBb = (function () {
    var C = CryptoJS;
    let CFBb = C.lib.BlockCipherMode.extend(), WordArray = C.lib.WordArray;
    /**
    ...
    var bitshift = function (wordArray, n) {
        var carry = 0, words = wordArray.words, wres, skipped = 0, carryMask;
        if (n > 0) {
            while (n > 31) {
    ...
    function processBlock(words, offset, encryptor) {
        ...
        var slidingSegmentMask = fullSegmentMask.clone(), slidingSegmentMaskShifted = slidingSegmentMask.clone(), slidingNegativeSegmentMask, prevCT;
        ...
        for (i = 0; i < blockSize / segmentSize; i++) {
            if (!prev) {
                ...
                prev = prev.words;
                let previousCiphertextSegment = self._ct;
...
const CRYPTO_ALGORITHM = 'aes-256-cfb8'; // 8 bits segment size
const DEFAULT_IV = "0123456789123456";
function encryptTextWithKey(valueToEncrypt, secretKey) {
    const key = CryptoJS.enc.Utf8.parse(CryptoJS.MD5(secretKey).toString());
...
exports.encryptTextWithKey = encryptTextWithKey;
